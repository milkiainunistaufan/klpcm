...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
  <h1 class="h2"><a href="/dashboard/klpcmbpjs/create" class="btn btn-success">Tambah data</a></h1>
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
  <h2>KLPCM BPJS</h2>
  <div class="btn-toolbar mb-2 mb-md-0">
    <a href="/dashboard/klpcmbpjss/cetak{{ request()->filled('dpjp_id') ? '?dpjp_id='.trim(request('dpjp_id')) : '' }}" target="_blank" class="btn btn-success">Cetak</a>
  </div>
</div>
  ...
  <table class="table table-striped table-sm border-gray-500">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          <form action="/dashboard/klpcmbpjs/carabayar/{{ $klpcm->id }}" method="post" class="d-inline">
            @csrf
            <input type="text" hidden value="umum" name="cara_bayar">
            <button class="badge bg-success border-0" >bpjs</button>
          </form>
        </td>
        <td>
          <a class="badge bg-warning text-decoration-none" href="/dashboard/klpcmbpjs/{{ $klpcm->id }}/edit">Edit</a>
        </td>
        <td>
          <form action="/dashboard/klpcmbpjs/{{ $klpcm->id }}" method="post" class="d-inline">
            @method('delete')
            @csrf
