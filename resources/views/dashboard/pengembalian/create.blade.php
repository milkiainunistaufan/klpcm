...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
  ...
  <div class="col-lg-7">
      <form action="/dashboard/pengembalian" method="post">
        @csrf
          <div class=" row">
            ...
            <label for="kelamin" class="col-sm-3 col-form-label">Kelamin</label>
                <div class="col-sm-7">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" {{ (old("kelamin") == 'lk' ? "checked":"") }} type="radio" name="kelamin" id="kelamin" value="lk">
                        <label class="form-check-label" for="kelamin">Laki - laki</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" {{ (old("kelamin") == 'pr' ? "checked":"") }} type="radio" name="kelamin" id="kelamin" value="pr">
            ...
            <label for="dpjp_id" class="col-sm-3 col-form-label">Peminjam</label>
                <div class="col-sm-7 mb-3">
                    <select class="form-select form-select-sm" name="dpjp_id" aria-label="Small select example">
                        ...
                        <option value="{{ $dpjp->id }}" {{ (old("dpjp_id") == $dpjp->id ? "selected":"") }}>{{ $dpjp->dokter }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-sm-7">
                    <select class="form-select form-select-sm" name="ruang_id" aria-label="Small select example">
                        @foreach ($ruangs as $ruang)
                            <option value="{{ $ruang->id }}" {{ (old("ruang_id") == $ruang->id ? "selected":"") }}>{{ $ruang->nama_ruang }}</option>  
                        @endforeach
            ...
            <div class="col-sm-7 mb-3">
              <input type="date" name="tgl_pengembalian" class="form-control @error('tgl_pengembalian') is-invalid @enderror" id="tgl_pengembalian" required value="{{ old('tgl_pengembalian') }}">
            ...
            <div class="col-sm-7 mb-3">
              <input type="date" name="tgl_kembali_rm" class="form-control @error('tgl_kembali_rm') is-invalid @enderror" id="tgl_kembali_rm" required value="{{ old('tgl_kembali_rm') }}">
            </div>
                <div class="col-sm-7 mb-3">
